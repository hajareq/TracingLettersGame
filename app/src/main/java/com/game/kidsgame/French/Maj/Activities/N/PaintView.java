package com.game.kidsgame.French.Maj.Activities.N;
...
import static com.game.kidsgame.French.Maj.Activities.N.N.maxX;
import static com.game.kidsgame.French.Maj.Activities.N.N.maxY;
...
public class PaintView extends View {
    ...
    private void touchStart(float x, float y) {
        ...
        else if(paths.size()==2 && x<((maxX*0.83)+50) && x>((maxX*0.83)-50) && y<((maxY*0.88)+50) && y>((maxY*0.88)-50))
        {
            paths.add(fp);
            current=3;
        }
    ...
    private void touchMove(float x, float y) {
        ...
        if(paths.size()==2 && current==2 &&((x>0.5*maxX-30 && x<0.5*maxX+30 && (y<0.47*maxY-50 || y>0.47*maxY+50 ) )|| x<((maxX*0.22)-70) || y<((maxY*0.05)-70))){
            paths.remove(paths.size()-1);
            mPath.reset();
            invalidate();
            mCanvas.drawColor(Color.TRANSPARENT, PorterDuff.Mode.CLEAR);
        ...
        if(paths.size()==2 && current==2 && y>0.47*maxY-30 && y<0.47*maxY+30 && (x<0.5*maxX-50 || x>0.5*maxX+50 ) ){
            paths.remove(paths.size()-1);
            mPath.reset();
            invalidate();
            mCanvas.drawColor(Color.TRANSPARENT, PorterDuff.Mode.CLEAR);
        ...
        if(paths.size()==3 && current==3 && ((y>0.50*maxY-50 && y<0.50*maxY+50 && (x<0.83*maxX-50 || x>0.83*maxX+50)) || x<((maxX*0.83)-70) || y>((maxY*0.88)+70))){
            paths.remove(paths.size()-1);
            mPath.reset();
            invalidate();
            mCanvas.drawColor(Color.TRANSPARENT, PorterDuff.Mode.CLEAR);
    ...
    private void touchUp() {
        ...
        if(paths.size()==1 && current==1 && !(mX<(((maxX)*0.22)+50) && mX>((maxX*0.22)-50) && mY<(((maxY)*0.88)+50) && mY>((maxY*0.88)-50))){
            paths.remove(paths.size()-1);
            mPath.reset();
            invalidate();
            mCanvas.drawColor(Color.TRANSPARENT, PorterDuff.Mode.CLEAR);
        ...
        if(paths.size()==2 && current==2 &&  !(mX<(((maxX)*0.83)+50) && mX>((maxX*0.83)-50) && mY<(((maxY)*0.88)+50) && mY>((maxY*0.88)-50))) {
            paths.remove(paths.size()-1);
            mPath.reset();
            invalidate();
            mCanvas.drawColor(Color.TRANSPARENT, PorterDuff.Mode.CLEAR);
