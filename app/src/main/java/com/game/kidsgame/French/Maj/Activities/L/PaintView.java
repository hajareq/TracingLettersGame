package com.game.kidsgame.French.Maj.Activities.L;
...
import static com.game.kidsgame.French.Maj.Activities.L.L.maxX;
import static com.game.kidsgame.French.Maj.Activities.L.L.maxY;
...
public class PaintView extends View {
    ...
    private void touchStart(float x, float y) {
        ...
        else if(paths.size()==1 && x<((maxX*0.22)+50) && x>((maxX*0.22)-50) && y<((maxY*0.88)+50) && y>((maxY*0.88)-50))
        {
            paths.add(fp);
            current=2;
    ...
    private void touchMove(float x, float y) {
        ...
        if(paths.size()==1 && current==1 && y>0.50*maxY-50 && y<0.50*maxY+50 && (x<0.22*maxX-50 || x>0.22*maxX+50)){
        ...
        if(paths.size()==2 && current==2 && x>0.5*maxX-30 && x<0.5*maxX+30 && (y<0.88*maxY-50 || y>0.88*maxY+50 ) ){
            paths.remove(paths.size()-1);
            mPath.reset();
            invalidate();
            mCanvas.drawColor(Color.TRANSPARENT, PorterDuff.Mode.CLEAR);
    ...
    private void touchUp() {
        ...
        if(paths.size()==2 && current==2 &&  !(mX<(((maxX)*0.85)+50) && mX>((maxX*0.85)-50) && mY<(((maxY)*0.88)+50) && mY>((maxY*0.88)-50))) {
            paths.remove(paths.size()-1);
            mPath.reset();
            invalidate();
            mCanvas.drawColor(Color.TRANSPARENT, PorterDuff.Mode.CLEAR);
