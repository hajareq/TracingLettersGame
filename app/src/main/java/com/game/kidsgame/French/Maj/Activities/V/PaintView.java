package com.game.kidsgame.French.Maj.Activities.V;
...
import static com.game.kidsgame.French.Maj.Activities.V.V.maxX;
import static com.game.kidsgame.French.Maj.Activities.V.V.maxY;
...
public class PaintView extends View {
    ...
    private void touchStart(float x, float y) {
        ...
        if(paths.size()==0 && x<((maxX*0.18)+50) && x>((maxX*0.18)-50) && y<((maxY*0.1)+50) && y>((maxY*0.1)-50))
        {
            paths.add(fp);
            current=1;
        }
    ...
    private void touchMove(float x, float y) {
        ...
        if(paths.size()==1 && current==1 && ((y<0.5*maxY+50 && y>0.5*maxY-50 && (x<0.35*maxX-70 || (x>0.35*maxX+70 && x<0.72*maxX-70) || x>0.72*maxX+70)) || y<0.15*maxX-60)){
        ...
        if(paths.size()==1 && current==1 && x<0.5*maxX+30 && x>0.5*maxX-30 && (y<0.9*maxY-70 || (y>0.9*maxY+70) ) ){
            paths.remove(paths.size()-1);
            mPath.reset();
            invalidate();
            mCanvas.drawColor(Color.TRANSPARENT, PorterDuff.Mode.CLEAR);
